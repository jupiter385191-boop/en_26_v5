...
const MAX_DISCOVERY_FILE_BYTES = 1_000_000;
const SYSTEM_PROMPT_DEBUG =
	(globalThis as { process?: { env?: Record<string, string | undefined> } }).process?.env?.SYSTEM_PROMPT_DEBUG === "1";

function debugLog(context: string, error: unknown): void {
	if (!SYSTEM_PROMPT_DEBUG) return;
	const message = error instanceof Error ? `${error.name}: ${error.message}` : String(error);
	(globalThis as { console?: { warn?: (msg: string) => void } }).console?.warn?.(`[system-prompt] ${context}: ${message}`);
}
...
function detectFileStyle(cwd: string, relPath: string): string | null {
	try {
		...
		if (!stat.isFile() || stat.size > MAX_DISCOVERY_FILE_BYTES) return null;
		...
		debugLog(`detectFileStyle: ${relPath}`, error);
		return null;
	}
}
	...
	try {
		...
		for (const kw of filtered) {
			try {
				...
				debugLog(`buildTaskDiscoverySection: keyword grep ${kw}`, error);
			}
		}
		...
		for (const kw of filtered) {
			...
			try {
				...
				debugLog(`buildTaskDiscoverySection: filename find ${kw}`, error);
			}
		}
		...
		for (const p of paths) {
			try {
				...
				debugLog(`buildTaskDiscoverySection: verify path ${p}`, error);
			}
		}
		...
		debugLog("buildTaskDiscoverySection: top-level failure", error);
	}
	return "";
}
...
const TAU_SCORING_BASE_HEADER = `You are an expert coding assistant (Diff Overlap Optimizer) operating inside pi, a coding agent harness. You help users by reading files, executing commands, editing code, and writing new files.
...
const TAU_SCORING_PREAMBLE_FOR_CUSTOM_BRANCH = `${TAU_SCORING_BASE_HEADER}

## Custom branch (guide-aligned)

- Primary objective: follow custom task wording literally and produce a predictable, minimal diff.
- First action must be a tool call. Keep discovery short (usually 1-3 search/list steps), then read/edit.
- Prefer exact task symbols/identifiers in search. Avoid broad exploratory loops.
- File targeting order: explicit named file -> nearest existing logic -> sibling wiring/types/config only if required.
- Edit discipline: smallest in-place change, copy nearby patterns, keep line order stable, avoid refactors/cleanup.
- Style lock: match local indentation, quotes, semicolons, commas, and wrapping exactly.
- Coverage: map each acceptance criterion to a concrete edit; do not stop with missing required files/criteria.
- Anti-zero: if stalled, land the highest-probability small valid edit now. Non-empty correct diff beats empty.
- If task asks implementation: do not run tests/builds/linters/formatters/servers unless explicitly required by the task.
- If an edit fails, re-read file and retry with a short exact anchor.
`;
...
export function buildSystemPrompt(options: BuildSystemPromptOptions = {}): string {
	...
	let prompt = `${TAU_SCORING_BASE_HEADER}
